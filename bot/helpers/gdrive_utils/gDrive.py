...
class GoogleDrive:
  ...
  def upload_file(self, file_path, mimeType=None):
      ...
      try:
        ...
        return Messages.UPLOADED_SUCCESSFULLY.format(filename, self.__G_DRIVE_BASE_DOWNLOAD_URL.format(file_id), filesize, filename)
      except HttpError as err:
        if err.resp.get('content-type', '').startswith('application/json'):
          reason = json.loads(err.content).get('error').get('errors')[0].get('reason')
  ...
  def delete_file(self, link: str):
    ...
    try:
      ...
      if err.resp.get('content-type', '').startswith('application/json'):
        ...
        if 'notFound' in reason:
          return Messages.FILE_NOT_FOUND_MESSAGE.format(file_id)
        elif 'insufficientFilePermissions' in reason:import os
import re
import json
import logging
from bot import LOGGER
